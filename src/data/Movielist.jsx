...
export function MovieCard() {
  return (
    <div className="mt-7 w-full grid grid-cols-4 gap-6 h-full">
      {movies.map((movie, index) => (
        <div
          key={index}
          className=" bg-white border-0 overflow-x-hidden rounded-2xl "
        >
          <div className="relative h-64">
            ...
            <p className="absolute top-3 right-4 text-white bg-purple p-2 border-0 rounded-lg text-lg font-semibold leading-none">
              {movie.rating}
            </p>
          </div>

          <div className="px-5 pt-6 pb-8 ">
            <p className="font-semibold text-xl mb-1">{movie.name}</p>
            <div className="flex justify-between mb-3">
              <p className="flex items-center gap-2 text-sm leading-0 text-gray">
                <Image src="/icons/calendar.svg" width={14} height={14} />
            ...
            <p className="text-gray text-sm mb-4">${movie.price}</p>
            <div>
              <button>
                <Image src="/icons/edit.svg" width={14} height={14} />
              </button>
              <button>
                <Image src="/icons/bin.svg" width={14} height={14} />
              </button>
